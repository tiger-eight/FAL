...
class FAL(nn.Module):
    ...
    def forward(self, pred, true, epoch):
        ...
        gamma1 = 1+b*(1-epoch)/(epoch+b)
        gamma2 = a * x * x + (1 - a) * x
        alpha_factor = true * self.alpha + (1 - true) * (1 - self.alpha)
        modulating_factor = (1.0 - p_t) ** (gamma1+gamma2)
        loss *= alpha_factor * modulating_factor
...
class ComputeLoss:
    ...
    def __init__(self, model,  autobalance=False):
        ...
        if g < 0:
            BCEcls, BCEobj = FAL(BCEcls, g), FAL(BCEobj, g) #RAL
    ...
    def __call__(self, p, targets, epoch):
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            b, a, gj, gi = indices[i]  # image, anchor_id, gridy, gridx
            ...
            if n:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    if self.gamma == 0:
                        ...
                        lcls += self.BCEcls(pcls, t, epoch)  # new focal loss
            ...
            if self.gamma == 0:
                ...
                obji = self.BCEobj(pi[..., 4], tobj,epoch)
